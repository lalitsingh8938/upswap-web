...
const VendorDocument = () => {
  ...
  const handleDocChange = async (e) => {
    ...
    localStorage.setItem("uploaded_business_documents1", JSON.stringify([...businessDocs, ...validDocs]));
  };

  const handlePhotoChange = async (e) => {
    const files = Array.from(e.target.files);
    const uploadedPhotos = await Promise.all(files.map(uploadPhoto));
    const validPhotos = uploadedPhotos.filter(Boolean).flat().map((photo) => photo?.compressed);
    setBusinessPhotos((prevPhotos) => [...prevPhotos, ...validPhotos]);
    localStorage.setItem("uploaded_images1", JSON.stringify([...businessPhotos, ...validPhotos]));
  };
  ...
  const removeDocument = (index) => {
    ...
    localStorage.setItem("uploaded_business_documents1", JSON.stringify(updatedDocs));
  };
