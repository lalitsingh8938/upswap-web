  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://api.upswap.app/api/send-otp/",
        {
          phone_number: phone, // 'phone' state should now contain number without country code
        }
      );
      ...
      if (response.status === 200) {
        ...
        toast.success("OTP sent successfully!");
        setTimeout(() => navigate("/VerifyOtpForgotPassword"), 2000);
      } else {
        toast.error("Something went wrong. Try again!");
      }
    } catch (error) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-white">
      ...
      <div className="w-full max-w-md px-6 shadow-lg rounded-md">
          ...
          <div className="relative flex items-center border rounded-lg p-2">
            <PhoneInput
              ...
              onChange={(value, countryData) => {
                let numberWithoutDialCode = value;
                if (countryData && countryData.dialCode) {
                  numberWithoutDialCode = value.replace(`+${countryData.dialCode}`, '');
                }
            ...
            <span className="absolute right-4 text-gray-400 text-sm">
              {phone.length}/10 {/* Adjust based on expected length */}
            </span>
          </div>
        </div>
        ...
        <button
          onClick={handleSubmit}
          className={`w-full text-white text-lg py-3 rounded-lg mt-6 mb-6 ${
            phone.length < 7 || loading // Adjust minimum length as needed
              ? "bg-gray-400 cursor-not-allowed"
              : "bg-orange-500 hover:bg-orange-600"
          }`}
          disabled={phone.length < 7 || loading} // Adjust minimum length as needed
        >
          {loading ? "Sending..." : "Submit"}
        </button>
      </div>
